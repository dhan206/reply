var rl, readline = require('readline'); // requires readline, a module for command line input/output
...
var confirm = exports.confirm = function(message, callback) {
  ...
  var question = {
    'reply': {
      type: 'confirm', // type confirm, yes or no answers only accepted
      message: message,
      default: 'yes'
    }
  }
...
var get = exports.get = function(options, callback) {
  ...
  if (typeof options != 'object') // returns an error if an invalid option object is given
  ...
  // gets default values of the questions in the option
  ...
  // guesses the type of reply, either yes/no or a free response 
  ...
  // used to check if the passed answer is of the valid type or form
  ...
  // gives an error if the value is invalid, shows the avaliable options for answers
  ...
  var wait_for_password = function(prompt, callback) {
    ...
    var keypress_callback = function(c, key) {
      ...
      if (key && (key.name == 'enter' || key.name == 'return')) { // new line
        stdout.write("\n");
      ...
      if (key && key.ctrl && key.name == 'c') // to close prompt
        close_prompt();
  ...
  // checks if the reply is valid, either goes onto next question or error message
  ...
  // returns whether the depends_on conditions have been met in the option
  ...
  var next_question = function(index, prev_key, answer) {
    ...
    if (options[curr_key].depends_on) { // current option contains depends_on property
      if (!dependencies_met(options[curr_key].depends_on))
        return next_question(++index, curr_key, undefined);
    }

    var prompt = (options[curr_key].type == 'confirm') ? // current option is of type confirm
      ' - yes/no: ' : " - " + curr_key + ": ";
    ...
    if (options[curr_key].type == 'password') {
      ...
      wait_for_password(prompt, function(reply) { // used to mask password input
  ...
  // passes given answers and error message to callback function once the readline is closed.
